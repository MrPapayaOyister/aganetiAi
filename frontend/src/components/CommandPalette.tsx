                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors
                      ${isActive ? 'bg-[#00D4FF]/10' : 'hover:bg-white/[0.03]'}`}
                  >
                    <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0
                      ${isActive ? 'bg-[#00D4FF]/15 text-[#00D4FF]' : 'bg-white/[0.04] text-[#94A3B8]'}`}>
                      <Icon size={15} />
                    </div>
                    <span className={`text-sm flex-1 ${isActive ? 'text-[#E2E8F0]' : 'text-[#94A3B8]'}`}>
                      {c.label}
                    </span>
                    {c.hint && <span className="text-[11px] text-[#4A6080]">{c.hint}</span>}
                    {isActive && <CornerDownLeft size={13} className="text-[#00D4FF]" />}
                  </button>
                )
              })}
            </div>
