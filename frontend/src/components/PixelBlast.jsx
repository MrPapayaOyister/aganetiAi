...
// React Bits PixelBlast — extended with forwardRef so the wrapper can
// trigger ripples programmatically from agent state (no DOM-click hack).
...
const PixelBlast = forwardRef(function PixelBlast(props, ref) {
  const {
    variant = 'square',
    pixelSize = 3,
    color = '#B497CF',
  ...
  // Imperative API — used by AgentPixelField to trigger non-pointer ripples.
  useImperativeHandle(ref, () => ({
    /** Trigger a ripple at normalized coords (0..1, top-left origin). */
    triggerRipple(nx = 0.5, ny = 0.5) {
    ...
    setColor(hex) {
      const t = threeRef.current
      if (!t) return
      t.uniforms.uColor.value.set(hex)
    },
    setSpeed(v) {
      speedRef.current = v
    },
    ...
    if (mustReinit) {
      ...
      if (liquid) {
        ...
        liquidEffect = createLiquidEffect(touch.texture, {
          strength: liquidStrength, freq: liquidWobbleSpeed,
        })
